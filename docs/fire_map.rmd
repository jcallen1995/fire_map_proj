...
#why are all the polygons corrupt?
fire_data_perim_corrupt_test <- st_read(fire_data_path) %>% 
  st_as_sf() %>% 
  #okay, so I' having issues with small polygons in the multipoly
  #I'll try unioning them
  dplyr::select(YEAR_, FIRE_NAME, geometry) %>% 
...
fire_corrupt_buffer_test <- st_buffer(fire_data_perim_corrupt_test[!is.na(fire_data_perim_corrupt_test)], 0.0)


fire_data_corrupt <- as.data.frame(st_is_valid(fire_data_perim_corrupt_test))

fire_data_corrupt_2 <- as.data.frame(st_is_valid(st_set_crs(fire_data_perim_corrupt_test, 4326)))

fire_data_corrupt_3 <- as.data.frame(st_is_valid(fire_data_perim_corrupt_test), sf_use_s2(FALSE))
...
fire_2016_corrupt <- as_data_frame(st_is_valid(fire_data_2016))
...
fire_2017_corrupt <- as_data_frame(st_is_valid(st_set_crs(fire_data_2017$geometry, 4326)))

print(fire_data_2017)
plot(fire_data_2017)
...
map = leaflet() %>%
  ...
  addPolygons(data = cali_counties_final, 
              ...
              weight = 1, 
              color = "grey", 
              group = "County Borders") %>% #county border polys
  

  addRasterImage(x = precip_conus_leaf, 
                 opacity = .3, 
  ...
  addLayersControl(
    ...
    overlayGroups = c("County Borders", "Precipitation", "Fires in 2016"),
