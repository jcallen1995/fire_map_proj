...
#let's see what the raw fire data has that could help me
fire_data_select_test_full <- fire_data_2017 %>% #okay, from here I'm going to try to add a column containing the county names the fire intersects
  st_transform(crs = "+proj=longlat +datum=WGS84") %>%
  st_make_valid() %>% #had to make the fire data clean because of the conversion from geos to s2 and duplicate points
...
impact_graph <- ggplot(data = fire_data_select_test_full, aes(x = date, y = roll_7))+
  geom_col(col = "red")+
  facet_wrap(~state)+
...
map = leaflet() %>%
  ...
  addMiniMap() %>% 
  addMarkers(lng=-117, lat=39, popup = popupGraph(test_graph), icon = graph_icon)
